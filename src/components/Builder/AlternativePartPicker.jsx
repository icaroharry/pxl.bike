...
const AlternativePartPicker = ({ part, selectAlternative, pixelSize }) => {
  ...
  useEffect(() => {
    Object.keys(pixels[part]).forEach((p, i) => {
      ...
      if (canvas && canvas.getContext) {
        ...
        const [x, y] = getDimensions(pixels[part][p], pixelSize).map(pixel =>
          Math.floor(pixel / 2)
        );
  ...
  }, [part, squareDimension, pixelSize]);

  return (
    partsToDraw.length > 0 && (
      <div className="flex flex-row sm:flex-col align-middle justify-center p-5 bg-white my-0 sm:my-3 rounded shadow">
        {partsToDraw}
      </div>
    )
  );
